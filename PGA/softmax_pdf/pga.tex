...
设 logits 向量 $\bm z=(z_1,\dots,z_C)^\top$，softmax 定义为
\[
\hat y_k(\bm z)\;=\;\frac{e^{z_k}}{\sum_{j=1}^C e^{z_j}},\qquad k=1,\dots,C
\]
...
即
\[
\mathrm{softmax}(\bm z+c\,\bm 1) \;=\; \mathrm{softmax}(\bm z)\quad(\forall c)
\]
令交叉熵损失
\[
L(\bm z)\;=\;-\sum_{k=1}^C y_k \log \hat y_k(\bm z)
\]
（其中 $\bm y$ 为目标分布，one-hot 时 $\sum_k y_k=1$）。由于 $L$ 只依赖 $\hat{\bm y}$，
\[
L(\bm z+c\,\bm 1)\;=\;L(\bm z)\quad(\forall c)
\]
因此沿着 $\bm 1$ 方向，$L$ 为常数，故
\[
\nabla_{\bm z} L(\bm z)^\top \bm 1 \;=\; 0,\qquad
H(\bm z)\,\bm 1 \;=\; \bm 0,\qquad
\bm 1^\top H(\bm z)\,\bm 1 \;=\; 0
\]
另外，已知
\[
\nabla_{\bm z} L(\bm z) \;=\; \hat{\bm y}(\bm z)-\bm y,
\]
于是
\[
\nabla_{\bm z} L(\bm z)^\top \bm 1
= \sum_{k=1}^C (\hat y_k - y_k)
= \underbrace{\sum_{k=1}^C \hat y_k}_{=\,1}
- \underbrace{\sum_{k=1}^C y_k}_{=\,1}
= 0
\]
对任意方向 $\bm v\in\mathbb R^C$，
\[
\bm v^\top H(\bm z)\,\bm v
= \sum_{k=1}^C \hat y_k v_k^2 \;-\; \Big(\sum_{k=1}^C \hat y_k v_k\Big)^2
= \mathrm{Var}_{k\sim \hat{\bm y}}(v_k)\;\ge\;0
\]
...
\]
因此 $H(\bm z)\succeq 0$，
当 $\bm v$ 为常数向量（如 $\bm v=\bm 1$）时，方差为 0，故该方向的二阶导为 0，
说明 $L$ 为凸但非严格凸，  
...
\]
范数上界,把 $\bm W$ 看成一个线性放大器，这个最大放大倍数就叫谱范数也等于最大奇异值：
\[
...
\[
\Big\|\pdv{L}{\bm x}\Big\|
= \|\bm W(\hat{\bm y}-\bm e_y)\|
\le \|\bm W\|_2\,\|\hat{\bm y}-\bm e_y\|_2
\le \sqrt2\,\|\bm W\|_2,
\qquad
\Big\|\pdv{L}{\bm w_j}\Big\|\le \|\bm x\|.
\]
在标准形式 \(z_j=\bm w_j^\top \bm x + b_j\) 下，若缺少归一化/正则，训练倾向通过增大
\(\|\bm W\|\)（有时也增大 \(\|\bm x\|\)）来放大 \(z_y - z_{k\neq y}\)，而非优化角度；这会提升
\(\|\bm W\|_2\) 最大放大倍数，谱范数，使
\[
\big\|\tfrac{\partial L}{\partial \bm x}\big\|
=\|\bm W(\hat{\bm y}-\bm e_y)\|
\;\le\; \|\bm W\|_2\,\|\hat{\bm y}-\bm e_y\|
\;\le\; \sqrt{2}\,\|\bm W\|_2
\]
...
% - 切向分量（与 \hat{x} 正交）只改变方向的一阶影响，对长度的一阶影响为 0；
...
特征与类别权重均作单位化：
\[
\bm x\in\mathbb{S}^{d-1},\quad
\hat{\bm w}_j=\frac{\bm w_j}{\|\bm w_j\|}\in\mathbb{S}^{d-1},\quad
z_j=s\,\hat{\bm w}_j^\top \bm x,\quad
p_j=\mathrm{softmax}_j(z).
\]
单位球面在 $\bm x$ 处的切空间与正交投影：
\[
T_{\bm x}\mathbb{S}^{d-1}=\{v\in\mathbb{R}^d: \bm x^\top v=0\},\qquad
\Pi_{\bm x}:=I-\bm x\bm x^\top.
\]
注意对任意由 logits 组合得到的损失 $L(\{z_j\})$，链式法则给出：
\[
\frac{\partial L}{\partial \bm x}
=\sum_j \frac{\partial L}{\partial z_j}\frac{\partial z_j}{\partial \bm x}
=\sum_j (p_j-\mathbb{1}_{j=y})\; s\;\frac{1}{\|\bm x\|}\big(I-\hat{\bm x}\hat{\bm x}^\top\big)\,\hat{\bm w}_j
\]\[
=\frac{s}{\|\bm x\|}\Pi_{\bm x} \sum_j (p_j-\mathbb{1}_{j=y})\,\hat{\bm w}_j
\]
\subsection{ArcFace 的切向合力}
\[
\hat{\bm x}=\frac{\bm x}{\|\bm x\|},\quad \hat{\bm w}_j=\frac{\bm w_j}{\|\bm w_j\|},\quad
...
\[
\hat{\bm w}_y
= (\hat{\bm w}_y^\top \hat{\bm x})\,\hat{\bm x}
+ \big(\hat{\bm w}_y - (\hat{\bm w}_y^\top \hat{\bm x})\,\hat{\bm x}\big)
= \cos\theta_y\,\hat{\bm x} + \Pi_{\bm x}\,\hat{\bm w}_y
\]
% (3) 切向分量的长度
\[
\|\Pi_{\bm x} \hat{\bm w}_y\|^2
= \|\hat{\bm w}_y - \cos\theta_y\,\hat{\bm x}\|^2
= 1 + \cos^2\theta_y - 2\cos\theta_y(\hat{\bm w}_y^\top \hat{\bm x})\]\[
= 1 - \cos^2\theta_y
= \sin^2\theta_y
\]
\[
\Rightarrow\quad
\|\Pi_{\bm x} \hat{\bm w}_y\|=\sin\theta_y,
\qquad
\Pi_{\bm x} \hat{\bm w}_y=\sin\theta_y\,\bm t_y,
\quad
\bm t_y:=\frac{\Pi_{\bm x} \hat{\bm w}_y}{\|\Pi_{\bm x} \hat{\bm w}_y\|}
\]
% 关键微分：角度与余弦对 x 的梯度
\[
\frac{\partial \cos\theta_j}{\partial \bm x}=\frac{1}{\|\bm x\|}\,\Pi_{\bm x}\,\hat{\bm w}_j,
\qquad
\frac{\partial \theta_y}{\partial \bm x}=-\frac{1}{\|\bm x\|}\,\bm t_y
\]
\[
\cos\theta_j=\hat{\bm w}_j^\top \hat{\bm x},\ \ 
\frac{\partial \hat{\bm x}}{\partial \bm x}
=\frac{1}{\|\bm x\|}\big(I-\hat{\bm x}\hat{\bm x}^\top\big)=\frac{1}{\|\bm x\|}\Pi_{\bm x}\]\[
\Rightarrow
\frac{\partial \cos\theta_j}{\partial \bm x}
=\Big(\frac{\partial \hat{\bm x}}{\partial \bm x}\Big)^\top \hat{\bm w}_j
=\frac{1}{\|\bm x\|}\Pi_{\bm x}\hat{\bm w}_j
\]
% ===== 式子 2：∂ θ_y / ∂ x =====
\[
\theta_y=\arccos(\hat{\bm w}_y^\top \hat{\bm x})
\Rightarrow
\frac{\partial \theta_y}{\partial \bm x}
=\frac{d\,\arccos(u)}{du}\bigg|_{u=\cos\theta_y}\cdot
\frac{\partial \cos\theta_y}{\partial \bm x}\]\[
= -\frac{1}{\sin\theta_y}\cdot \frac{1}{\|\bm x\|}\Pi_{\bm x}\hat{\bm w}_y
\]
又因为
\(
\sin\theta_y=\sqrt{1-(\hat{\bm w}_y^\top \hat{\bm x})^2}
= \|\Pi_{\bm x}\hat{\bm w}_y\|
\),
故:
\[
\frac{\partial \theta_y}{\partial \bm x}
=-\frac{1}{\|\Pi_{\bm x}\hat{\bm w}_y\|}\cdot \frac{1}{\|\bm x\|}\Pi_{\bm x}\hat{\bm w}_y
=-\frac{1}{\|\bm x\|}\,\frac{\Pi_{\bm x}\hat{\bm w}_y}{\|\Pi_{\bm x}\hat{\bm w}_y\|}
=-\frac{1}{\|\bm x\|}\,\bm t_y
\]
% 真类（带 margin）logit 的链式法则
\[
\frac{\partial z_y}{\partial \bm x}
=s\frac{\partial \cos(\theta_y+m)}{\partial \bm x}
=s\big(-\sin(\theta_y+m)\big)\frac{\partial \theta_y}{\partial \bm x}
=\frac{s}{\|\bm x\|}\,\sin(\theta_y+m)\,\bm t_y
\]
将 \(\bm t_y\) 换回 \(\Pi_{\bm x}\hat{\bm w}_y\) 得到“共线缩放”：
\[
\boxed{\;
\frac{\partial z_y}{\partial \bm x}
=\frac{s}{\|\bm x\|}\,\underbrace{\frac{\sin(\theta_y+m)}{\sin\theta_y}}_{:=\alpha(\theta_y,m)}
\,\Pi_{\bm x}\hat{\bm w}_y
\;}
\]
可见方向与 \(\Pi_{\bm x}\hat{\bm w}_y\) 共线，仅被 \(\alpha(\theta_y,m)\) 缩放；当 \(m=0\) 时 \(\alpha=1\)
% 负类（无 margin）logit
\[
\frac{\partial z_j}{\partial \bm x}
=s\frac{\partial \cos\theta_j}{\partial \bm x}
=\frac{s}{\|\bm x\|}\,\Pi_{\bm x}\,\hat{\bm w}_j,\qquad j\ne y
\]
...
\frac{\partial L}{\partial \bm x}
&=\sum_j \frac{\partial L}{\partial z_j}\frac{\partial z_j}{\partial \bm x}\\
&=(p_y-1)\frac{s}{\|\bm x\|}\alpha(\theta_y,m)\Pi_{\bm x}\hat{\bm w}_y
\;+\;\sum_{j\ne y} p_j\,\frac{s}{\|\bm x\|}\Pi_{\bm x}\hat{\bm w}_j\\
&=\frac{s}{\|\bm x\|}\,\Pi_{\bm x}
\Big(\sum_{j\ne y}p_j\,\hat{\bm w}_j - \alpha(\theta_y,m)\,\hat{\bm w}_y\Big)
\end{aligned}
\]
梯度完全落在 \(\bm x\) 的切空间内（被 \(\Pi_{\bm x}\) 投影），
“真类吸引”与“切向方向”严格共线，margin 仅通过 \(\alpha(\theta_y,m)\) 调整同一方向的力度
\subsection{PGA K/Z 对齐的切向合力}
对同类且可靠的邻居（经掩码与 EMA 平滑）集合 $\mathcal N_y(\bm x)$，K/Z 对齐的一个统一写法是最大化方向一致性（或最小化某相似度损失）：
\[
L_{\mathrm{pga}}
=\sum_{a\in\mathcal N_y(\bm x)} \beta_a\,\phi(\langle \bm x,\ \bm x_a^{\mathrm{tgt}}\rangle),
\qquad \beta_a\ge 0,\ \ \phi'(\cdot)\le 0
\]
其中 $\bm x_a^{\mathrm{tgt}}$ 来自下层/EMA 的目标方向，于是
\[
\pdv{L_{\mathrm{pga}}}{\bm x}
=\sum_a \beta_a\,\phi'(\langle \bm x,\bm x_a^{\mathrm{tgt}}\rangle)\,\bm x_a^{\mathrm{tgt}}
= -\sum_a \alpha_a\,\bm x_a^{\mathrm{tgt}}\quad(\alpha_a:=\beta_a|\phi'|\ge 0)
\]
投影到切空间得
\begin{equation}
\label{eq:pga-grad}
-\bm g_{\mathrm{pga}}
\ :=\ -\Pi_{\bm x}\pdv{L_{\mathrm{pga}}}{\bm x}
\ =\ \sum_{a\in\mathcal N_y(\bm x)} \alpha_a\,\Pi_{\bm x} \bm x_a^{\mathrm{tgt}}
\end{equation}
即 PGA 的下降方向是同类目标邻居在切空间的加权均值

\subsection{切空间内积判断对抗性}
ArcFace 与 PGA 的是否“冲突”取决于它们在切空间的内积：
\begin{equation}
\label{eq:ip-test}
\left\langle -\bm g_{\mathrm{arc}},\ -\bm g_{\mathrm{pga}}\right\rangle
=\Big\langle \Pi_{\bm x} \bm u,\ \sum_{a} \alpha_a\,\Pi_{\bm x} \bm x_a^{\mathrm{tgt}}\Big\rangle
\end{equation}
\noindent 若该内积 $\ge 0$，两者同向/弱同向，不对抗；若 $<0$，则局部相悖，令
\[
\theta_a:=\angle\!\big(\Pi_{\bm x} \bm u,\ \Pi_{\bm x} \bm x_a^{\mathrm{tgt}}\big),\qquad
c_a:=\cos\theta_a
\]
由柯西不等式，
\[
\Big\langle \Pi_{\bm x} \bm u,\ \sum_a \alpha_a\,\Pi_{\bm x} \bm x_a^{\mathrm{tgt}}\Big\rangle
=\|\Pi_{\bm x} \bm u\|\sum_a \alpha_a\,\|\Pi_{\bm x} \bm x_a^{\mathrm{tgt}}\|\,c_a\]\[
\ \ge\
\|\Pi_{\bm x} \bm u\|\Big(\sum_a \alpha_a\,\|\Pi_{\bm x} \bm x_a^{\mathrm{tgt}}\|\Big)\cdot \min_a c_a
\]
因此，只要
\begin{equation}
\label{eq:sufficient}
\boxed{\ \min_{a\in\mathcal N_y(\bm x)} \angle\!\big(\Pi_{\bm x} \bm u,\ \Pi_{\bm x} \bm x_a^{\mathrm{tgt}}\big)\ \le\ 90^\circ\ }
\end{equation}
就有 $\langle -\bm g_{\mathrm{arc}},-\bm g_{\mathrm{pga}}\rangle\ge 0$，充分保证两者不对抗，
