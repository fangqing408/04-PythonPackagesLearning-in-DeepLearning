...
def _knn_mask(sim, base_mask, topk):
    # B 也就是 batch 的大小
    B = sim.size(dim=0) 
    # 去掉自环，不许把自己选成自己的邻居，对角线上减去了一个极大的数字，后续 topk 不会选到他
    # 把不允许成为侯选边的设置为极小值，不会被 topk 取到
    sim_no_diag = sim - torch.eye(B, device=config.device) * 1e9
...
class PGAHead(nn.Module):
    def __init__(self, num_layers, topk=8, t_diff=1, use_ema_target=True):
        super().__init__()
    ...
    def _graph(self, F512, labels, a, b):
        S = pairwise_cosine(F512)
        A_intra, A_inter = build_intra_inter(S, labels=labels, topk=self.topk)
        # 加一点自环，避免度为 0
        A = a * A_intra + b * A_inter + 1e-6 * torch.eye(S.size(0), device=config.device)
        A_norm = normalize_sym(A)
        K = diffusion_kernel(A_norm, t_diff=self.t_diff)
        return A_norm, K
    ...
    def forward(self, feats_final, labels, lambda_align_K=64, lambda_align_Z=16, lambda_idea=1.0, sigma_in=0.99, sigma_out=0.01, stopgrad=True, ema_m=0.9):
        L = len(feats_final)
        
        A_list, K_list, Z_list = [], [], []
        for i, (F512, gam) in enumerate(zip(feats_final, self.gams)):
            a_i, b_i = self._get_alpha_beta(i)
            Ai, Ki = self._graph(F512, labels, a_i, b_i)
            Zi = gam(Ai, F512)
        ...
        if self.use_ema_target:
            ...
            with torch.no_grad():
                for i in range(L):
                    self._ema_K[i].data.mul_(ema_m).add_(K_list[i].detach() * (1.0 - ema_m))
